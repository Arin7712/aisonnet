  ...
  useEffect(() => {
    const updateLineHeight = () => {
      if (ref.current && lastItemRef.current) {
        ...
        // Ensure height calculation respects mobile devices
        if (window.innerWidth < 768) {
          setLineHeight(lastItemRect.bottom - containerRect.top - 40); // Adjust for spacing
        } else {
          setLineHeight(lastItemRect.bottom - containerRect.top);
        }
      }
    };
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto py-10 px-4 md:px-8 lg:px-10">
        <h2 className="text-2xl font-medium md:text-4xl mb-4 text-black dark:text-white max-w-4xl">
          Our 4 Stage Process
        </h2>
        <p className="text-neutral-700 dark:text-gray-400 font-medium text-sm md:text-base max-w-sm">
